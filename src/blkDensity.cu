...
void spmv_kernel_new(uint64_t* first_blocks, uint64_t* A_keys, ValueIn* A_values, ValueIn* v, ValueOut* u, uint64_t* A_bmps, uint64_t* A_offsets, int n){
	...
	while(blocksLeft>0)
	{
		...
		// if(blocksLeft>3){
		// 	nnz = A_offsets[last - blocksLeft + 4] - prevElems;
		// }else{
		// 	nnz = A_offsets[last] - prevElems;
		// }

		// if(thread_id < nnz){
		// 	shmem[thread_id] = A_values[prevElems + thread_id];
		// }
		...
		// res += (ValueOut) shmem[threadIdx.y*32 + threadIdx.x]*shmem[8*32 + threadIdx.x];
		//res += (ValueOut) shmem[threadIdx.x]*shmem[8*32 + blk*8 + col];
		blocksLeft-=4;					
	}
...
inline void bmSparse_SpMV(bmSpMatrix<ValueIn> &A, ValueIn* v, ValueOut* u, std::ofstream& file){
	...
	//spmv_kernel<<<A.num_cols/8 + 1, 64>>>(offsets, A_keys_raw, A_values_raw, v, u, A_bmps_raw, first_values_A, A.num_cols);
...
__device__ int bmpCount(uint64_t d){
  return (__popcll(d));
}
...
int main(int argc, char** argv) {
	...
	//char* matrix;
	//matrix = (char *) malloc(strlen(argv[2])+1);
	//strcpy(matrix, argv[2]);	
	...
	bmSpMatrix<half> A_bmSp(A_path, false);	

	//std::cout << "matriz: " << std::string(matrix) << std::endl;
	...
	//for(thrust::device_vector<uint64_t, thrust::device_allocator<uint64_t>>::iterator iter = A_bmSp.bmps.begin(); iter != A_bmSp.bmps.end(); iter++){
	//	hist[cntBits(static_cast<uint64_t>(*iter))]++;
	//}


	
	for(int j=0; j<A_bmSp.bmps.size(); j++){
	//for(int j=0; j<1; j++){
	//	cant = cntBits(A_bmSp.bmps[j]);
	//	hist[cant]++;
		hist[cntBits(A_bmSp.bmps[j])]++;
	}

	

	//for(int k=0; k<65; k++){
	//	printf("hist en %d es %d\n", k, hist[k]); 
	//}

	//long hist2[1000];
	
	std::ofstream myfile;
      	myfile.open ("histogramas.csv", std::ios_base::app);
